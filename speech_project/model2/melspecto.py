"""
Urban Sound Classification — PyTorch CNN (Mel Spectrograms)
===========================================================
Features
--------
  • Log-Mel Spectrogram feature extraction with zero-padding
  • 4-block CNN (mirrors original Keras architecture)
  • Full training loop with checkpointing
  • Results file  →  results_melspec/results_summary.txt
  • Report-quality plots saved to  results_melspec/plots/
  • 8-bit Post-Training Quantization (PTQ)  →  saved_models_melspec/model_int8.pt
  • Grad-CAM heatmaps over Mel spectrograms
...
CHECKPOINT_PATH = "saved_models_melspec/weights_best_cnn.pt"
INT8_PATH       = "saved_models_melspec/model_int8.pt"
RESULTS_DIR     = "results_melspec"
...
def extract_features(file_name: str) -> "np.ndarray | None":
    """Load an audio file → zero-padded Log-Mel Spectrogram matrix."""
    try:
        ...
        pad_w = MAX_PAD_LEN - log_melspec.shape[1]
        if pad_w < 0:
            log_melspec = log_melspec[:, :MAX_PAD_LEN]
        else:
            log_melspec = np.pad(log_melspec, ((0, 0), (0, pad_w)), mode="constant")
...
def build_feature_dataframe(metadata_csv: str, audio_root: str) -> pd.DataFrame:
    ...
    for _, row in metadata.iterrows():
        ...
        if feat is not None:
            ...
            rows.append({"feature": feat, "class_label": label, "file_path": fp})
...
def train(model, train_loader, val_loader, num_epochs):
    ...
    for ep in range(1, num_epochs + 1):
        ...
        print(f"  Ep {ep:3d}/{num_epochs} | TrainLoss={tl:.4f} Acc={ta*100:.2f}% | ValLoss={vl:.4f} Acc={va*100:.2f}%")
        if va > best_val_acc:
            best_val_acc = va
...
def apply_ptq(model_fp32: nn.Module) -> nn.Module:
    ...
    model_int8 = quantize_dynamic(model_int8, {nn.Linear}, dtype=torch.qint8)
...
class GradCAM:
    ...
    def _fwd_hook(self, _, __, output): self.activations = output.detach()
    def _bwd_hook(self, _, __, grad_output): self.gradients = grad_output[0].detach()
    ...
    def compute(self, tensor: torch.Tensor, class_idx: int) -> np.ndarray:
        ...
        cam_img = Image.fromarray(cam).resize((MAX_PAD_LEN, N_MELS), resample=Image.BILINEAR)
...
def plot_class_distribution_bar(y_raw: np.ndarray, class_names: list):
    counts = pd.Series(y_raw).value_counts().reindex(class_names).fillna(0)
    ...
    ax.set_xlabel("Sound Class", fontsize=11); ax.set_ylabel("Number of Samples", fontsize=11)
    ax.set_title("UrbanSound8K — Samples per Class", fontsize=13, fontweight="bold")
    plt.xticks(rotation=35, ha="right", fontsize=9)
    plt.tight_layout()
    _savefig("class_distribution_bar.png")

def plot_melspec_grid(features_df: pd.DataFrame, class_names: list):
    n, nc = len(class_names), 5
    nr = (n + nc - 1) // nc
    ...
    for i, cls in enumerate(class_names):
        ...
        if subset.empty: axes[i].axis("off"); continue
        melspec = subset.iloc[0]["feature"]
        axes[i].imshow(melspec, aspect="auto", origin="lower", cmap="magma")
        axes[i].set_title(cls, fontsize=9, fontweight="bold")
        axes[i].set_xlabel("Time Frames", fontsize=7); axes[i].set_ylabel("Mel Bands", fontsize=7)
        axes[i].tick_params(labelsize=6)

    for j in range(i + 1, len(axes)): axes[j].axis("off")
    fig.suptitle("Log-Mel Spectrograms — One Sample per Class", fontsize=13, fontweight="bold", y=1.01)
    plt.tight_layout()
    _savefig("melspec_sample_grid.png")
...
def plot_confusion_matrix(y_true, y_pred, class_names):
    cm_data = confusion_matrix(y_true, y_pred, normalize="true")
    fig, ax = plt.subplots(figsize=(11, 9))
    sns.heatmap(cm_data, annot=True, fmt=".2f", cmap="Blues",
                xticklabels=class_names, yticklabels=class_names,
                linewidths=0.5, linecolor="lightgrey", ax=ax, annot_kws={"size": 8})
...
def plot_per_class_f1(y_true, y_pred, class_names):
    ...
    ax.set_ylim(0, 1.12)
    ax.set_xlabel("Sound Class", fontsize=11); ax.set_ylabel("F1 Score", fontsize=11)
...
def plot_quantization_comparison(fp32_acc, int8_acc, fp32_latency_ms, int8_latency_ms, fp32_size_mb, int8_size_mb):
    ...
    ax.bar_label(b1, fmt="%.2f", fontsize=8, padding=3); ax.bar_label(b2, fmt="%.2f", fontsize=8, padding=3)
...
def plot_gradcam_grid(gradcam: GradCAM, features_df: pd.DataFrame, le: LabelEncoder, class_names: list):
    n, nc = len(class_names), 5
    nr = (n + nc - 1) // nc
    ...
    for i, cls in enumerate(class_names):
        ...
        if subset.empty: axes[i].axis("off"); continue
        melspec = subset.iloc[0]["feature"]
        tensor = torch.tensor(melspec[np.newaxis, np.newaxis, :, :], dtype=torch.float32)
        class_idx = int(le.transform([cls])[0])
        cam = gradcam.compute(tensor, class_idx)

        axes[i].imshow(melspec, aspect="auto", origin="lower", cmap="gray", alpha=0.65)
        axes[i].imshow(cam,  aspect="auto", origin="lower", cmap="jet",  alpha=0.55)
        axes[i].set_title(cls, fontsize=8, fontweight="bold")
        axes[i].set_xlabel("Time Frames", fontsize=6); axes[i].set_ylabel("Mel Band", fontsize=6)
    ...
    fig.suptitle("Grad-CAM Heatmaps over Log-Mel Spectrograms", fontsize=12, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def write_results(class_names, y_true, y_pred, train_acc, test_acc, fp32_latency, int8_latency, fp32_size, int8_size, int8_acc, history):
    ...
    w(f"  FP32 model size     : {fp32_size:.3f} MB")
    w(f"  INT8 model size     : {int8_size:.3f} MB")
    w(f"  Size reduction      : {(1 - int8_size/fp32_size)*100:.1f}%")
    w(f"  FP32 latency (CPU)  : {fp32_latency:.3f} ms / sample")
    w(f"  INT8 latency (CPU)  : {int8_latency:.3f} ms / sample")
    w(f"  Latency speedup     : {fp32_latency/int8_latency:.2f}×")
    ...
    text = buf.getvalue()
    with open(RESULTS_TXT, "w") as f: f.write(text)
    print(text)
...
def main():
    ...
    print(f"\n{'='*60}\n  Urban Sound Classification  |  device: {DEVICE}\n{'='*60}\n")

    print("[1/9] Extracting Log-Mel Spectrogram features …")
    ...
    plot_melspec_grid(features_df, class_names)
    ...
    sample_melspec = X_test[0]

    fp32_latency = benchmark_latency(model_cpu,  sample_melspec)
    int8_latency = benchmark_latency(model_int8, sample_melspec)
    ...
    plot_quantization_comparison(test_acc, int8_acc, fp32_latency, int8_latency, fp32_size, int8_size)
